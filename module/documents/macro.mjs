  ...
  switch ( dropData.type ) {
    case "Activity":
      ...
      foundry.utils.mergeObject(macroData, {
        ...
        img: activity.img,
        command: `varlyn5e.documents.macro.rollItem("${activity.item._source.name}", { activityName: "${
          activity._source.name}", event });`,
        flags: { "varlyn5e.itemMacro": true }
      });
      break;
    case "Item":
      ...
      foundry.utils.mergeObject(macroData, {
        name: itemData.name,
        img: itemData.img,
        command: `varlyn5e.documents.macro.rollItem("${itemData._source.name}", { event })`,
        flags: { "varlyn5e.itemMacro": true }
      });
      break;
    case "ActiveEffect":
      ...
      foundry.utils.mergeObject(macroData, {
        ...
        img: effectData.img,
        command: `varlyn5e.documents.macro.toggleEffect("${effectData.name}")`,
        flags: { "varlyn5e.effectMacro": true }
      });
      break;
    default:
      return;
