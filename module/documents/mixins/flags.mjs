...
export default function SystemFlagsMixin(Base) {
  class SystemFlags extends Base {
    ...
    prepareData() {
      super.prepareData();
      if ( ("varlyn-dnd5e" in this.flags) && this._systemFlagsDataModel ) {
        this.flags["varlyn-dnd5e"] = new this._systemFlagsDataModel(this._source.flags["varlyn-dnd5e"], { parent: this });
      }
    }
    ...
    async setFlag(scope, key, value) {
      if ( (scope === "varlyn-dnd5e") && this._systemFlagsDataModel ) {
        let diff;
        const changes = foundry.utils.expandObject({ [key]: value });
        if ( this.flags["varlyn-dnd5e"] ) diff = this.flags["varlyn-dnd5e"].updateSource(changes, { dryRun: true });
        else diff = new this._systemFlagsDataModel(changes, { parent: this }).toObject();
        return this.update({ flags: { "varlyn-dnd5e": diff } });
      }
