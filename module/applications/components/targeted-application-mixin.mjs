...
export default function TargetedApplicationMixin(Base) {
  return class TargetedApplicationElement extends Base {
    ...
    buildTargetContainer() {
      ...
      if ( !this.chatMessage?.getFlag("varlyn-dnd5e", "targets")?.length ) this.targetSourceControl.hidden = true;
    ...
    buildTargetsList() {
      ...
      switch ( this.targetingMode ) {
        case "targeted":
          this.chatMessage?.getFlag("varlyn-dnd5e", "targets")?.forEach(t => targetedTokens.set(t.uuid, t.name));
          break;
        case "selected":
